	...
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if activeMatch != nil {
			...
			if activeMatch.StartTime != nil {
				var var_21 string = activeMatch.StartTime.Format("Jan 02 - 3:04 PM")
			...
			var var_22 string = F("vs %s @ %s", activeMatch.Opponent, activeMatch.Location)
	...
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if len(approvedPFines) > 0 {
			...
			if qp.FineListOpen {
				...
				for _, pf := range approvedPFines {
					...
					var var_29 string = fmt.Sprintf("$%v - %s - %s", pf.Amount, pf.Reason, pf.Context)
		...
		for _, p := range players {
			...
			for _, f := range p.Fines {
				...
				var var_52 string = fmt.Sprintf("$%.0f", f.Amount)
				...
				if f.FineAt.After(twoWeeksAgo) {
					var var_53 string = humanize.Time(f.FineAt)
					_, err = templBuffer.WriteString(templ.EscapeString(var_53))
					if err != nil {
						return err
					}
				} else {
					var var_54 string = f.FineAt.Format("2006-01-02")
	...
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if len(createdFines) > 0 {
			if len(createdFines) == 1 {
				...
				var var_59 string = fmt.Sprintf("Created %d Fines:", len(createdFines))
			...
			for _, cf := range createdFines {
				...
				var var_60 string = fmt.Sprintf("%+v", cf.PlayerID)
	...
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if isOpen {
			...
			if isFineMaster {
				...
				var var_67 templ.SafeURL = makeSafeUrlWithAnchor(baseUrl, false, false, false, false, false, "fine-add")
			...
			var var_70 templ.SafeURL = makeSafeUrlWithAnchor(baseUrl, false, true, false, false, false, "fine-add")
			...
			if isFineMaster {
				var_71 := `Fine a Player`
				_, err = templBuffer.WriteString(var_71)
				if err != nil {
					return err
				}
			} else {
				var_72 := `Suggest a Fine`
...
func fineAdd(baseUrl string, isOpen bool, players []PlayerWithFines, presetFines []PresetFine, isFineMaster bool) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if isOpen {
			...
			var_78 := `-- !!! Fine is not listed here --`
			...
			var_79 := ` Dynamic options will be inserted here `
			...
			for _, fp := range presetFines {
				if fp.Approved {
					...
					var var_80 string = fmt.Sprintf("%s ($%v)", fp.Reason, fp.Amount)
					_, err = templBuffer.WriteString(templ.EscapeString(var_80))
					if err != nil {
						return err
					}
					_, err = templBuffer.WriteString("</option>")
			...
			var_89 := `-- no player - just suggest fineable offence --`
			...
			for _, p := range players {
				...
				var var_90 string = fmt.Sprintf("%s", p.Name)
			...
			var var_91 = []any{bigAdd}
			err = templ.RenderCSSItems(ctx, templBuffer, var_91...)
			if err != nil {
				return err
			}
			...
			_, err = templBuffer.WriteString(templ.EscapeString(templ.CSSClasses(var_91).String()))
			if err != nil {
				return err
			}
			_, err = templBuffer.WriteString("\">")
			...
			var var_94 templ.SafeURL = makeSafeUrlWithAnchor(baseUrl, false, false, false, false, false, "fine-add")
			...
			var var_97 templ.SafeURL = makeSafeUrlWithAnchor(baseUrl, false, true, false, false, false, "fine-add")
			...
			if isFineMaster {
				var_98 := `Fine a Player`
				_, err = templBuffer.WriteString(var_98)
				if err != nil {
					return err
				}
			} else {
				var_99 := `Suggest a Fine`
