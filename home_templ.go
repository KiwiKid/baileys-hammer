...
func home(players []PlayerWithFines, approvedPFines []PresetFine, pendingPFines []PresetFine, fineWithPlayers []FineWithPlayer, qp HomeQueryParams) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		var var_16 string = GetTitle(ctx)
		...
		if len(approvedPFines) > 0 {
			...
			if qp.FineListOpen {
				...
				var var_23 templ.SafeURL = makeSafeUrlWithAnchor(baseUrl, !qp.FineListOpen, false, false, false, false, "fines")
				...
				var_27 := `Pending Fines `
				_, err = templBuffer.WriteString(var_27)
				if err != nil {
					return err
				}
				var var_28 string = fmt.Sprintf("(%d)", len(pendingPFines))
		...
		for _, p := range players {
			...
			var var_36 string = fmt.Sprintf("$%d (%d)", p.TotalFines, p.TotalFineCount)
			...
			if UseRoles(ctx) {
				if len(p.Role) > 0 {
					...
					var var_42 string = p.RoleDescription
			...
			for _, f := range p.Fines {
				...
				var var_46 string = fmt.Sprintf("$%.0f", f.Amount)
				...
				if f.FineAt.After(twoWeeksAgo) {
					var var_47 string = humanize.Time(f.FineAt)
					_, err = templBuffer.WriteString(templ.EscapeString(var_47))
					if err != nil {
						return err
					}
				} else {
					var var_48 string = f.FineAt.Format("2006-01-02")
