...
func finemaster(pass string, players []PlayerWithFines, fineWithPlayers []FineWithPlayer, pFines []PresetFine, matches []Match, qp FineMasterQueryParams) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		_, err = templBuffer.WriteString("</div><div class=\"mx-auto p-4 hidden\" id=\"quick-finer\"><div class=\"flex items-center justify-center bg-gray-100 mx-auto\"><ul>")
		if err != nil {
			return err
		}
...
func matchesList(isOpen bool, matches []Match, pwfs []PlayerWithFines) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if isOpen {
			_, err = templBuffer.WriteString("<div class=\"w-full px-8 py-6 text-left bg-white shadow-xl\" id=\"match-list\"><h3 class=\"text-2xl font-bold text-center\" hx-get=\"/#preset-fine\" hx-push-url=\"/#preset-fine\">")
			if err != nil {
				return err
			}
			var_67 := `🚧 Matches 🚧`
			...
			_, err = templBuffer.WriteString("</h3><div class=\"mx-auto bg-gray-200 shadow-xl m-10 flex flex-col justify-center items-center\"><div class=\"w-full\">")
			if err != nil {
				return err
			}
			for _, m := range matches {
				_, err = templBuffer.WriteString("<div hx-trigger=\"load once\" hx-get=\"")
				if err != nil {
					return err
				}
				_, err = templBuffer.WriteString(templ.EscapeString(fmt.Sprintf("/match/%d?type=list", m.ID)))
				if err != nil {
					return err
				}
			...
			var var_71 = []any{F("%s float-right", bigPri)}
...
func matchesManage(baseUrl string, isOpen bool, matches []Match, pwfs []PlayerWithFines) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		if isOpen {
			...
			_, err = templBuffer.WriteString("<div class=\"mx-auto bg-gray-200 shadow-xl m-10 flex flex-col justify-center items-center\">")
			if err != nil {
				return err
			}
			for _, m := range matches {
			...
			var var_86 templ.SafeURL = makeSafeUrlWithAnchorV2(baseUrl, "manage-matches", true)
